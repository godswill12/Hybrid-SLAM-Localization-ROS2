...
class EIFNode(Node):
    def __init__(self):
        ...
        self.pose_pub = self.create_publisher(PoseStamped, '/pose_eif', 10)
        self.mu    = np.zeros(3)
        self.Omega = np.eye(3) * 10.0
        self.xi    = self.Omega @ self.mu
        self.Q     = np.diag([0.05, 0.05, 0.02])
        self.R_imu = np.array([[0.01]])
        self.last_time    = None
        self.imu_yaw_rate = 0.0
        self.get_logger().info('EIF node started')
    ...
    def predict(self, v, w, dt):
        ...
        mu_bar  = np.array([x + v*cos(theta)*dt, y + v*sin(theta)*dt, theta + w*dt])
        P_bar   = F @ P @ F.T + self.Q
        self.Omega = np.linalg.inv(P_bar)
        self.xi    = self.Omega @ mu_bar
        self.mu    = mu_bar

    def update_imu(self, imu_w, dt):
        H     = np.array([[0.0, 0.0, 1.0 / dt]])
        R_inv = np.linalg.inv(self.R_imu)
        z     = np.array([imu_w])
